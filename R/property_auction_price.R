...
get_one_page <- function(url){
  ...
  df <- mutate(
    ...
    rating_value=as.numeric(gsub("[^[:digit:]]", "", lapply(rating_value, get_rating_value))),
    rating_dates=str_sub(my(rating_dates),1,7)
  )
  df
}
...
get_number <- function(region){
  num_url <- paste0("https://www.interest.co.nz/property/residential-auction-results?region=",
...
replace_blank_with_na <- function(x,y){
